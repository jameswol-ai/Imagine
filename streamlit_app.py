...
# ------------------------------------------------------------
# CORE FUNCTIONS (same as original but enhanced)
# ------------------------------------------------------------
...
def generate_building_model(domain,btype,floors,baths,country,material_frame,plot_size,soil_type,g_k,q_k,steel_section,seismic_zone,wind_zone,username):
    room_map={"Luxury Villa":["Bedroom","Bedroom","Bedroom","Living Room","Kitchen","Bathroom","Dining","Office"],
...
def compute_detailed_forex_boq(design, rate_overrides=None):
    """Compute BoQ. rate_overrides can be a dict with keys like 'concrete', 'steel', etc. to adjust unit rates."""
...
def draw_2d_blueprint(design, overlay_design=None):
    """Draw floor plan. If overlay_design provided, show semi-transparent overlay for comparison."""
    layout=design["layout"]["grid"]; nx=design["layout"]["nx"]; ny=design["layout"]["ny"]
    fig,ax=plt.subplots(figsize=(8,8*ny/nx if nx>0 else 8)); ax.set_xlim(0,nx); ax.set_ylim(0,ny); ax.set_aspect('equal'); ax.axis('off')
    for i in range(ny):
        for j in range(nx):
...
def draw_3d_isometric_view(design, drift_factor=0):
    """3D stack with optional drift offset (for animation)."""
    layout=design["layout"]["grid"]; ny=len(layout); nx=len(layout[0]) if layout else 0
    floors=design["floors"]
    ...
    for f in range(floors):
        z=f*3.0; offset_x=drift_factor*math.sin(z/2)  # sway in X
        for i in range(ny):
            for j in range(nx):
                room=layout[i][j]; color=ROOM_COLORS.get(room,"#94a3b8")
...
def generate_ifc_json(design):
    """Create a JSON structure resembling IFC/Revit elements."""
